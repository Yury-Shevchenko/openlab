...
function checkNotificationSubscription() {
  ...
  navigator.serviceWorker.register('/service-worker.js').then(function () {
    ...
    .then(sub => {
      if(sub === null){
        if(notificationStatus) notificationStatus.innerText = "Please subscribe to receive notifications about the tests";
        if(enableNotificationsButton) enableNotificationsButton.style.display = "inline-block";
      } else {
        if(notificationStatus) notificationStatus.innerText = "You are subscribed to notifications";
        if(disableNotificationsButton) disableNotificationsButton.style.display = "inline-block";
      }
    })
    .catch(err => {
      console.error(err);
...
window.addEventListener('load', function() {
  if ('Notification' in window && 'serviceWorker' in navigator) {
    ...
    if(disableNotificationsButton) disableNotificationsButton.addEventListener('click', unsubscribeNotifications);
