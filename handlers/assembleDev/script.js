...
const { makeType } = require('./util/makeType');
const { adaptiveFunction } = require('./util/async');
  ...
  items
    ...
    .map(i => {
      ...
      if (['text', 'divider'].includes(i.type)) {
        ...
        return ({
          ...i,
          name: (i.name && i.name.replace(/\./g,'-')) || slugify((i.label && i.label.replace(/\./g,'-')) || '')
        })
      }
    })
...
exports.makeStudyTree = makeStudyTree
