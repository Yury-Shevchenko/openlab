...
exports.inviteParticipants = async (req, res) => {
  ...
  if (
    (sentEmails.length + newInvitationEmails.length >
      process.env.PROF_PLAN_INVITATIONS_LIMIT &&
      ...
      User.findById(req.user._id, (err, user) => {
        ...
        user.save((saveErr, updatedUser) => {
          if (saveErr) {
            ...
            req.flash(
              'success',
              `${res.locals.layout.flash_subscriptionSuccess}`
            );
          }
          res.redirect('/subscribe');
        });
  ...
  User.findById(req.user._id, (err, user) => {
    ...
    user.save((saveErr, updatedUser) => {
      if (saveErr) {
        ...
        req.flash(
          'success',
          `${res.locals.layout.flash_subscriptionReactivation}`
        );
      }
      res.redirect('back');
    });
